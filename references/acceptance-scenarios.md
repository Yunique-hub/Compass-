...
| AF | 用户在新会话说“继续上次那个非学习项目”。 | 从正确 workspace 和 checkpoint 恢复，不强行进入学生规划流程 |
| AG | 一个构建错误已复现、修复并通过测试；用户说以后别再犯。 | 在 `lessons.md` 保存问题、根因、修复、证据、范围和预防检查 |
| AH | 用户只在一次任务中要求极简回答。 | 仅记 observed；不把一次行为升级为跨项目永久偏好 |
| AI | 用户要求核验最新官方规则。 | 使用浏览工具时优先一手来源并记录日期；不可用时明确尚未验证 |
| AJ | 当前项目存在重复失败和明确 deadline。 | 主动给 1—3 个有证据的风险与最小验证动作，不声称后台监控 |
| AK | 非学习项目只授权当前工作区。 | 只写项目内 `.compass/`；不创建 home profile，不把局部状态冒充全局记忆 |
| AL | 新版启动时只发现旧 `~/.compass-student-growth` 档案。 | 恢复旧档案并提示迁移，不创建两个相互分叉的默认档案 |
