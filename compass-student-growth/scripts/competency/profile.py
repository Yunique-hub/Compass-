...
class CompetencyProfile:
    ...
    def apply_evidence(existing: Mapping[str, Any] | None, evidence: Mapping[str, Any]) -> dict[str, Any]:
        ...
        if evidence.get("verification_status") != "verified": return value
        ...
        evidence_ids = list(value.get("evidence", [])); evidence_ids.append(str(evidence["evidence_id"])); value["evidence"] = list(dict.fromkeys(evidence_ids))
